...
```sh
go run ./unigo-mapset unigo-mapset/testdata/input.txt

cat unigo-mapset/testdata/input.txt | go run ./unigo-mapset
```

## Why this one matters

`unigo` says "adopt the new type". `unigo-map` shows what today's code looks
...
`unigo` and `unigo-mapset` therefore run the same code. The difference between
...
The *call* costs nothing, though. [`unigo-map1`](../unigo-map1) writes
...
```sh
go test ./unigo-mapset                              # from the repository root
go build -o unigo-mapset/unigo-mapset ./unigo-mapset   # note the -o
```

`go build ./unigo-mapset` fails with `build output "unigo-mapset" already
