...
func TestMap(t *testing.T) {
	m := hash.NewMap[string, int](caseInsensitive{})
	...
	want1 := "{Hello: 2, World: 3}"
	want2 := "{World: 3, Hello: 2}"
	if s := m.String(); s != want1 && s != want2 {
		t.Errorf("String(): got %s, want %s or %s", s, want1, want2)
	}
