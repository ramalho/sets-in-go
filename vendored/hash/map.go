...
// Package hash defines hash-based containers such as [Map].
package hash
...
func NewMap[K, V any](hasher maphash.Hasher[K]) *Map[K, V] {
	var m Map[K, V]
	m.init(hasher)
	return &m
}
...
func (m *Map[K, V]) init(hasher maphash.Hasher[K]) {
...
func (m *Map[K, V]) ContainsAll(keys iter.Seq[K]) bool {
	return iter.Every(keys, m.Contains)
}
...
func (m *Map[K, V]) String() string {
	...
	slices.SortStableFunc(sorted, func(a, b *entry[K, V]) int {
		return fmtsort.Compare(a.key, b.key)
	})
